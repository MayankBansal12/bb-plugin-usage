...
OpenCode collection requires the `sqlite3` CLI on each enrolled machine that has an OpenCode database. The query is read-only, aggregates only usage metadata, is limited to 365 days and 900 KB of output, and times out after 60 seconds.
