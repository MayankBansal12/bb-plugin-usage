...
export function mergeLimitWindows(sources: ProviderLimitSource[]) {
  ...
  for (const source of sources) {
    for (const window of source.windows) {
      ...
      if (!current) {
        order.push(key);
        windows.set(key, { ...window, cost: window.cost ? { ...window.cost } : undefined });
        continue;
      }
...
/**
 * Subscription APIs report account-wide windows, even when queried through
 * different agent bridges or machines. Keep the fullest snapshot instead of
 * adding percentages (which would double-count the same account-wide usage),
 * and retain every machine snapshot as the audit trail beneath it.
 */
