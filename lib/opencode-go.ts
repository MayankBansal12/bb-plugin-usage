...
export function openCodeGoUsageCommand() {
  return [
    ...
    `bb_usage_go_key=''`,
    `bb_usage_go_parser=''`,
    `if command -v jq >/dev/null 2>&1; then bb_usage_go_parser='jq'; if ! jq -e 'type == "object"' "$auth_path" >/dev/null 2>&1; then printf '%s\\n' '__BB_USAGE_ERROR__:OpenCode auth file was not valid JSON.'; exit 1; fi; if ! jq -e '."opencode-go"? | type == "object" and .type == "api"' "$auth_path" >/dev/null 2>&1; then printf '%s\\n' '__BB_USAGE_ERROR__:no-opencode-go-credential'; exit 1; fi; bb_usage_go_key=$(jq -er '."opencode-go" | (.key // .api_key) | select(type == "string" and test("^[^[:space:]]+$"))' "$auth_path" 2>/dev/null); bb_usage_go_auth_status=$?; if [ "$bb_usage_go_auth_status" -ne 0 ]; then printf '%s\\n' '__BB_USAGE_ERROR__:OpenCode Go credential was invalid.'; exit 1; fi`,
    `elif command -v node >/dev/null 2>&1; then bb_usage_go_parser='node'; bb_usage_go_key=$(node -e 'let d;try{d=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"))}catch{process.exit(3)};if(!d||Array.isArray(d)||typeof d!=="object")process.exit(3);const e=d["opencode-go"];if(!e||Array.isArray(e)||typeof e!=="object"||e.type!=="api")process.exit(2);const k=e.key??e.api_key;if(typeof k!=="string"||!/^\\S+$/.test(k))process.exit(3);process.stdout.write(k)' "$auth_path" 2>/dev/null); bb_usage_go_auth_status=$?; if [ "$bb_usage_go_auth_status" -eq 2 ]; then printf '%s\\n' '__BB_USAGE_ERROR__:no-opencode-go-credential'; exit 1; fi; if [ "$bb_usage_go_auth_status" -ne 0 ]; then printf '%s\\n' '__BB_USAGE_ERROR__:OpenCode auth file or Go credential was invalid.'; exit 1; fi`,
